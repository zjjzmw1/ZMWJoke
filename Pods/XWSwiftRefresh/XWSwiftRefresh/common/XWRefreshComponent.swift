...
public class XWRefreshComponent: UIView {
    ...
    init(target:AnyObject, action:Selector){
        self.init()
        self.setRefreshingTarget(target: target, action: action)
    }
    ...
    public func beginRefreshing(){
        ...
        UIView.animate(withDuration: XWRefreshSlowAnimationDuration) { () -> Void in
            self.alpha = 1.0
        }
    ...
    */
    ...
    /** 8. 当scrollView的contentSize发生改变的时候调用 */ //NSKeyValueChangeKey : Any
    ...
    func executeRefreshingCallback(){
        ...
        dispatch_get_main_queue().asynchronously() { () -> Void in
            ...
            if let realTager = self.refreshingTarget {
                if realTager.responds(self.refreshingAction) == true {
                    
                    
                    let timer = Timer.scheduledTimerWithTimeInterval(timeInterval: 0, target: self.refreshingTarget, selector: self.refreshingAction, userInfo: nil, repeats: false)
                    RunLoop.mainRunLoop().addTimer(timer, forMode: RunLoopMode.commonModes)
                }
            }
    ...
    public override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if keyPath == XWRefreshKeyPathContentSize {
            self.scrollViewContentSizeDidChange(change: change as! Dictionary<String, AnyObject>?)
        }
        
        if self.isHidden {return}
        
        if keyPath == XWRefreshKeyPathContentOffset{
            self.scrollViewContentOffsetDidChange(change: change as! Dictionary<String, AnyObject>?)
        }else if keyPath == XWRefreshKeyPathPanKeyPathState{
            self.scrollViewPanStateDidChange(change: change as! Dictionary<String, AnyObject>?)
        }
    }
