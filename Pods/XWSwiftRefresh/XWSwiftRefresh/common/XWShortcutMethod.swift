...
public func xwDelay(time: TimeInterval, task:dispatch_block_t) -> xwTask? {
    ...
    let delayedClosure: xwTask = {
        ...
        if let realClosure = closure {
            ...
            if cancel == false {
                ...
                DispatchQueue.main.asynchronously(execute: realClosure)
            }
        }
        closure = nil
        result = nil
...
extension NSDate {
    ...
    func xwConvertStringTime() -> String {
        ...
        return "最后更新:" + formatter.string(from: self as Date)
