...
public class XWRefreshStateHeader: XWRefreshHeader {
    ...
    var closureCallLastUpdatedTimeTitle:((_ lastUpdatedTime:NSDate) -> String)?
    ...
    override func prepare() {
        super.prepare()
        //初始化文字
        self.setTitle(title: XWRefreshHeaderStateIdleText, state: .Idle)
        self.setTitle(title: XWRefreshHeaderStatePullingText, state: .Pulling)
        self.setTitle(title: XWRefreshHeaderStateRefreshingText, state: .Refreshing)
    ...
    override func placeSubvies() {
        ...
        if self.lastUpdatedTimeLabel.isHidden {
            ...
            //状态
            self.stateLabel.frame = CGRect.init(x: 0, y: 0, width: self.xw_width, height: self.xw_height*0.5)
