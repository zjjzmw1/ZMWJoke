...
public class XWRefreshHeader: XWRefreshComponent {
    ...
    public var lastUpdatedTime:NSDate{
        get{
            if let realTmp =  UserDefaults.standardUserDefaults().objectForKey(self.lastUpdatedateKey){
    ...
    override var state:XWRefreshState{
        ...
        didSet{
            ...
            if state == XWRefreshState.Idle {
                ...
                //保存刷新的时间
                UserDefaults.standardUserDefaults().setObject(NSDate(), forKey: self.lastUpdatedateKey as String)
