...
public class XWRefreshGifHeader: XWRefreshStateHeader {
    ...
    public func setImages(images:Array<UIImage>, state:XWRefreshState) -> Self {
        ...
        return self.setImages(images: images, duration: TimeInterval(images.count) * 0.1, state: state)
    }
    /** 设置刷新状态下,gif的图片,动画每帧相隔的时间 */
    public func setImages(images:Array<UIImage>, duration:TimeInterval, state:XWRefreshState) -> Self {
        //防止空数组 []
