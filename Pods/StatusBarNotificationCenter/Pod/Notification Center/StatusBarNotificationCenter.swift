...
public class StatusBarNotificationCenter: NSObject {
    ...
    var notificationViewFrame: CGRect {
        return CGRect.init(x: 0, y: 0, width: notificationViewWidth, height: internalnotificationViewHeight)
    }
    var notificationViewTopFrame: CGRect {
        return CGRect.init(x: 0, y: -internalnotificationViewHeight, width: notificationViewWidth, height: internalnotificationViewHeight);

    }
    var notificationViewLeftFrame: CGRect {
        return CGRect.init(x: -notificationViewWidth, y: 0, width: notificationViewWidth, height: internalnotificationViewHeight);
    ...
    var notificationViewBottomFrame: CGRect {
        return CGRect.init(x: 0, y: internalnotificationViewHeight, width: notificationViewWidth, height: 0);
    }
    ...
    //let notificationQ = dispatch_queue_create("notificationQueue", DISPATCH_QUEUE_SERIAL) //之前的串行队列
    let notificationQ = DispatchQueue(label: "notificationQueue") // 串行队列
    //let q = DispatchQueue(label: "queue", attributes: DispatchQueueAttributes.concurrent, target: nil) // 并发队列
